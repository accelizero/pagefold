...
export function request(method,params={},options={}) {
 return new Promise((resolve,reject)=>{
  ...
  socket.on('error',error=>finish(Error(['ENOENT','ECONNREFUSED'].includes(error.code)?'拾页尚未连接 Chrome。请加载拾页扩展，打开拾页 → 设置 → 连接 Codex → 重连。首次安装先运行 npm run setup:mcp。':error.message)));

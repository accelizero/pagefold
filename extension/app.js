...
const labels={overview:'统计总览',analysis:'AI 分析',projects:'常用项目',spaces:'工作空间',timeline:'关联线索',windows:'当前窗口',duplicates:'重复候选',history:'恢复记录',settings:'记录与隐私'};
const icons=['⌑','✳','◇','◌','◈','▧'];
...
function tabRow(t,{editable=false}={}) {
...
document.addEventListener('change',e=>{
 ...
 if(target.id==='mobile-nav') {if(target.value==='agent'){agentDialog();updateBridgeStatus();}else{view=target.value;render();}}
...
if(live) {
 ...
 chrome.storage.onChanged.addListener(changes=>{if(changes.analysis){analysis=changes.analysis.newValue||{status:'idle'};if(state)render();}if(changes.events||changes.paused||changes.projects||changes.historyImport||changes.analysisReports||changes.archivedGroups||changes.analysisSource) dirty();const d=changes.draft?.newValue;if(d&&state&&!busy&&d.sessionId===state.sessionId&&d.plan.fingerprint===fingerprint(state.tabs)){plan=d.plan;stale=false;render();}});
}
